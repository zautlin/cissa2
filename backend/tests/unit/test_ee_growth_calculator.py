"""
Unit tests for EE Growth SQL query generation

Tests the EEGrowthCalculator without requiring database connection.
"""
import pytest
from uuid import UUID
from backend.app.services.ee_growth_calculator import EEGrowthCalculator
from backend.app.models.ratio_metrics import MetricDefinition, MetricSource


# Sample metric definition for ee_growth
...
class TestEEGrowthCalculator:
    """Test SQL query generation for EE growth"""
    ...
    def test_calculator_initialization_3y(self):
        ...
        calc = EEGrowthCalculator(EE_GROWTH_METRIC, "3Y")
    ...
    def test_calculator_initialization_5y(self):
        ...
        calc = EEGrowthCalculator(EE_GROWTH_METRIC, "5Y")
    ...
    def test_calculator_initialization_10y(self):
        ...
        calc = EEGrowthCalculator(EE_GROWTH_METRIC, "10Y")
    ...
    def test_build_query_1y_single_ticker(self):
        ...
        assert "WITH ee_data AS" in sql_normalized
        assert "ee_rolling AS" in sql_normalized
        assert "ee_with_lag AS" in sql_normalized
        assert "LAG(ee_rolling_avg) OVER" in sql_normalized
        ...
        assert params["metric_name"] == "Calc EE"
    ...
    def test_query_includes_null_handling(self):
        ...
        assert "WHEN prior_year_avg_ee IS NULL THEN NULL" in sql
        
        # Verify zero denominator check
        assert "WHEN ABS(prior_year_avg_ee) = 0 THEN NULL" in sql
        
        # Verify growth calculation
        assert "(ee_rolling_avg - prior_year_avg_ee) / ABS(prior_year_avg_ee)" in sql
    
    def test_query_uses_metrics_outputs_table(self):
        """Test that query correctly sources from metrics_outputs"""
        calc = EEGrowthCalculator(EE_GROWTH_METRIC, "1Y")
        sql, params = calc.build_query(
            tickers=["CSL"],
            dataset_id=TEST_DATASET_ID,
        ...
        # Verify metrics_outputs table is used
        assert "FROM cissa.metrics_outputs" in sql
...
class TestEEGrowthSQLStructure:
    ...
    def test_complete_cte_chain(self):
        ...
        data_pos = sql.find("WITH ee_data AS")
        rolling_pos = sql.find("ee_rolling AS")
        lag_pos = sql.find("ee_with_lag AS")
        
        # Verify order
        assert data_pos > -1, "ee_data CTE missing"
        assert rolling_pos > data_pos, "ee_rolling CTE out of order"
        assert lag_pos > rolling_pos, "ee_with_lag CTE out of order"
