...
BEGIN
  ...
  WHERE
    ...
    AND f1.metric_value IS NOT NULL
    AND f2.metric_value IS NOT NULL;
END;
...
BEGIN
  ...
  SELECT
    f1.ticker,
    f1.fiscal_year,
    (f1.metric_value - f2.metric_value) AS calc_assets
  FROM cissa.fundamentals f1
  INNER JOIN cissa.fundamentals f2
    ON f1.ticker = f2.ticker
    AND f1.fiscal_year = f2.fiscal_year
...
BEGIN
  ...
  SELECT
    mo.ticker,
    mo.fiscal_year,
    (mo.metric_value - f1.metric_value - f2.metric_value) AS calc_oa
  ...
  WHERE
    ...
    AND mo.metric_name = 'Calc Assets'
    AND f1.metric_name = 'FIXED_ASSETS'
    AND f2.metric_name = 'GOODWILL'
    AND mo.metric_value IS NOT NULL
    AND f1.metric_value IS NOT NULL
    AND f2.metric_value IS NOT NULL;
END;
$$ LANGUAGE plpgsql IMMUTABLE;
...
BEGIN
  ...
  SELECT
    f1.ticker,
    f1.fiscal_year,
    (f1.metric_value - f2.metric_value) AS calc_op_cost
  FROM cissa.fundamentals f1
  INNER JOIN cissa.fundamentals f2
    ON f1.ticker = f2.ticker
    AND f1.fiscal_year = f2.fiscal_year
...
BEGIN
  ...
  SELECT
    f1.ticker,
    f1.fiscal_year,
    (f1.metric_value - f2.metric_value) AS calc_non_op_cost
  FROM cissa.fundamentals f1
  INNER JOIN cissa.fundamentals f2
    ON f1.ticker = f2.ticker
    AND f1.fiscal_year = f2.fiscal_year
...
BEGIN
  ...
  SELECT
    f1.ticker,
    f1.fiscal_year,
    (f1.metric_value - f2.metric_value) AS calc_tax_cost
  FROM cissa.fundamentals f1
  INNER JOIN cissa.fundamentals f2
    ON f1.ticker = f2.ticker
    AND f1.fiscal_year = f2.fiscal_year
...
BEGIN
  ...
  SELECT
    f1.ticker,
    f1.fiscal_year,
    (f1.metric_value - f2.metric_value) AS calc_xo_cost
  FROM cissa.fundamentals f1
  INNER JOIN cissa.fundamentals f2
    ON f1.ticker = f2.ticker
    AND f1.fiscal_year = f2.fiscal_year
...
BEGIN
  ...
  SELECT
    f1.ticker,
    f1.fiscal_year,
    (f1.metric_value / f2.metric_value) AS profit_margin
  FROM cissa.fundamentals f1
  INNER JOIN cissa.fundamentals f2
    ON f1.ticker = f2.ticker
    AND f1.fiscal_year = f2.fiscal_year
...
BEGIN
  ...
  SELECT
    mo.ticker,
    mo.fiscal_year,
    (mo.metric_value / f.metric_value) AS op_cost_margin
  FROM cissa.metrics_outputs mo
  ...
  WHERE
    mo.dataset_id = p_dataset_id
    AND mo.metric_name = 'Calc Op Cost'
    AND f.metric_name = 'REVENUE'
    AND mo.metric_value IS NOT NULL
    AND f.metric_value IS NOT NULL
    AND f.metric_value != 0;
END;
$$ LANGUAGE plpgsql IMMUTABLE;
...
BEGIN
  ...
  SELECT
    mo.ticker,
    mo.fiscal_year,
    (mo.metric_value / f.metric_value) AS non_op_cost_margin
  FROM cissa.metrics_outputs mo
  INNER JOIN cissa.fundamentals f
    ON mo.ticker = f.ticker
    AND mo.fiscal_year = f.fiscal_year
  ...
  WHERE
    mo.dataset_id = p_dataset_id
    AND mo.metric_name = 'Calc Non Op Cost'
    AND f.metric_name = 'REVENUE'
    AND mo.metric_value IS NOT NULL
    AND f.metric_value IS NOT NULL
    AND f.metric_value != 0;
...
BEGIN
  ...
  SELECT
    mo.ticker,
    mo.fiscal_year,
    (mo.metric_value / f.metric_value) AS eff_tax_rate
  FROM cissa.metrics_outputs mo
  INNER JOIN cissa.fundamentals f
    ON mo.ticker = f.ticker
    AND mo.fiscal_year = f.fiscal_year
  ...
  WHERE
    mo.dataset_id = p_dataset_id
    AND mo.metric_name = 'Calc Tax Cost'
...
BEGIN
  ...
  SELECT
    mo.ticker,
    mo.fiscal_year,
    (mo.metric_value / f.metric_value) AS xo_cost_margin
  FROM cissa.metrics_outputs mo
  INNER JOIN cissa.fundamentals f
    ON mo.ticker = f.ticker
    AND mo.fiscal_year = f.fiscal_year
  ...
  WHERE
    mo.dataset_id = p_dataset_id
    AND mo.metric_name = 'Calc XO Cost'
    AND f.metric_name = 'REVENUE'
    AND mo.metric_value IS NOT NULL
    AND f.metric_value IS NOT NULL
    AND f.metric_value != 0;
...
BEGIN
  ...
  SELECT
    f1.ticker,
    f1.fiscal_year,
    (f1.metric_value / f2.metric_value) AS fa_intensity
  FROM cissa.fundamentals f1
  INNER JOIN cissa.fundamentals f2
    ON f1.ticker = f2.ticker
    AND f1.fiscal_year = f2.fiscal_year
...
BEGIN
  ...
  SELECT
    f1.ticker,
    f1.fiscal_year,
    (f1.metric_value - f2.metric_value) AS book_equity
  FROM cissa.fundamentals f1
  INNER JOIN cissa.fundamentals f2
    ON f1.ticker = f2.ticker
    AND f1.fiscal_year = f2.fiscal_year
...
BEGIN
  ...
  SELECT
    mo.ticker,
    mo.fiscal_year,
    (f.metric_value / mo.metric_value) AS roa
  FROM cissa.metrics_outputs mo
  INNER JOIN cissa.fundamentals f
    ON mo.ticker = f.ticker
    AND mo.fiscal_year = f.fiscal_year
