...
# Build database URL from config with env var password fallback
...
@app.post("/api/v1/metrics/calculate-phase3", response_model=models.MetricsCalculateResponse)
async def calculate_metrics_phase3(request: models.MetricsCalculateRequest):
    ...
    return await handlers.handle_calculate_metrics_phase3(
        str(request.dq_id), request.parameters
    )
